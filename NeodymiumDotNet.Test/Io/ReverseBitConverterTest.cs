...
{
    public class ReverseBitConverterTest
    {
        public static IEnumerable<object[]> GetReadPrimitiveArgs()
        {
            ...
            yield return core(new byte[] { 0, 0, 0, 1 }, true, 1);
            yield return core(new byte[] { 0x01, 0x23, 0x45, 0x67 }, true, 0x01234567u);
            yield return core(new byte[] { 0x01, 0x23, 0x45, 0x67, 0x89 }, true, 0x01234567u);
            yield return core(new byte[] { 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef }, true, 0x01234567u);
            yield return core(new byte[] { 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0x01 }, true, 0x01234567u);
        }
        ...
        {
            var actualSucceeded = ReverseBitConverter.Instance.TryReadPrimitive<uint>(input, out var actualValue);
        ...
        public static IEnumerable<object[]> GetReadPrimitivesArgs()
        {
            ...
            yield return core(new byte[] { 0x01, 0x23, 0x45, 0x67 }, 1, true, new []{ 0x01234567u });
            yield return core(new byte[] { 0x01, 0x23, 0x45, 0x67, 0x89 }, 1, true, new []{ 0x01234567u });
            yield return core(new byte[] { 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef }, 2, true, new[] { 0x01234567u, 0x89abcdefu });
            yield return core(new byte[] { 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0x01 }, 2, true, new[] { 0x01234567u, 0x89abcdefu });
        ...
        public void ReadPrimitives(byte[] input, int dstSize, bool expectedSucceeded, uint[] expectedValues)
        {
            ...
            var actualSucceeded = ReverseBitConverter.Instance.TryReadPrimitives<uint>(input, actualValues);
            Assert.Equal(expectedSucceeded, actualSucceeded);
            if(actualSucceeded)
        ...
        public static IEnumerable<object[]> GetWritePrimitiveArgs()
        {
            ...
            yield return core(0x01234567u, 4, true, new byte[] { 0x01, 0x23, 0x45, 0x67 });
            yield return core(0x01234567u, 5, true, new byte[] { 0x01, 0x23, 0x45, 0x67, 0x00 });
        }
        ...
        public void WritePrimitive(uint input, int dstSize, bool expectedSucceeded, byte[] expectedValues)
        {
            ...
            var actualSucceeded = ReverseBitConverter.Instance.TryWritePrimitive(actualValues, input);
            Assert.Equal(expectedSucceeded, actualSucceeded);
            if(actualSucceeded)
                Assert.Equal(expectedValues, actualValues);
        }
        ...
        public static IEnumerable<object[]> GetWritePrimitivesArgs()
        {
            ...
            yield return core(new[] { 0x01234567u }, 4, true, new byte[] { 0x01, 0x23, 0x45, 0x67 });
            yield return core(new[] { 0x01234567u }, 5, true, new byte[] { 0x01, 0x23, 0x45, 0x67, 0x00 });
            yield return core(new[] { 0x01234567u, 0x89abcdefu }, 7, false, Array.Empty<byte>());
            yield return core(new[] { 0x01234567u, 0x89abcdefu }, 8, true, new byte[] { 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef });
            yield return core(new[] { 0x01234567u, 0x89abcdefu }, 9, true, new byte[] { 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0x00 });
        }
        ...
        {
            var actualValues = new byte[dstSize];
            var actualSucceeded = ReverseBitConverter.Instance.TryWritePrimitives<uint>(actualValues, input);
            Assert.Equal(expectedSucceeded, actualSucceeded);
            if(actualSucceeded)
                Assert.Equal(expectedValues, actualValues);
        }
